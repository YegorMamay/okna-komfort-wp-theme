...
<?php if(have_posts()): while(have_posts()): the_post();?>
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h5>
              <em>
                <span class="text-muted author"><?php _e('By', 'yegor'); echo " "; the_author() ?>,</span>
        ...
        <section>
            ...
            <?php the_excerpt( __( '&hellip; ' . __('Continue reading', 'yegor' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'yegor' ) ); ?>
        </section>
        <hr>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="fa fa-folder-open-o"></i>&nbsp; <?php _e('Category', 'yegor'); ?>: <?php the_category(', ') ?><br/>
                <i class="fa fa-comment-o"></i>&nbsp; <?php _e('Comments', 'yegor'); ?>: <?php comments_popup_link(__('None', 'yegor'), '1', '%'); ?>
            </p>
        </footer>
    </article>
<?php endwhile; ?>

<?php if ( function_exists('yegor_pagination') ) { yegor_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="fa fa-arrow-left"></i> ' . __('Previous', 'yegor')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'yegor') . ' <i class="fa fa-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
